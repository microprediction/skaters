...
effectively empty: three academic lines circle it — weighted conformal
(W1-ACAS, ICLR 2026), conformal + online FDR (Rebjock et al., C-PP-COAD,
e-LOND/e-GAI), and e-processes (e-detectors, PITMonitor) — in mutual
isolation (none cites SPOT/DSPOT, telemanom, or each other), and every
...
construction) and TAB sweeps thresholds and reports the best — an oracle,
not an alarm rule.
...
prefix anomaly-free — ~5M labeled-normal points; the FRED cache supplies
...
log P(not region) — the likelihood of a censored sample. Its price is a
theorem, not an accident: no proper score depends on tail shape alone
