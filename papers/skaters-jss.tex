...
A companion sweep on $2{,}500$ \proglang{FRED} series isolates the effect of the
...
correlated family (194 families):
...
\midrule
likelihood trunk + likelihood leaf & 85.2\,\% & 55.2\,\% & 3.44 \\
likelihood trunk + CRPS leaf       & \textbf{91.2\,\%} & \textbf{64.1\,\%} & \textbf{3.49} \\
bare CRPS leaf (no trunk)          & 80.6\,\% & 60.1\,\% & 2.96 \\
\bottomrule
\end{tabular}
\caption{Effect of the terminal-leaf objective. The middle row is the
\emph{model first, conform last} recipe (the \code{laplace} default: CRPS leaf,
lattice projection on). A general likelihood policy already edges conformal on
CRPS over independent families ($55.2\%$); repointing only the terminal leaf to
CRPS lifts that to $64.1\%$ and raises log-likelihood ($3.44 \rightarrow 3.49$),
matching the dedicated CRPS specialist while modelling faithfully.}
\label{tab:objective}
\end{table}
