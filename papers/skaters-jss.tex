...
single number, and which we recommend for price work. The library's committed
martingale-plus-clock model (Section~\ref{sec:doob}) is the natural \emph{shape}
of an in-library volatility specialist, but in its current form it does not match
\pkg{GARCH}-$t$ on price series; closing that gap is future work, not a present
claim.
...
The clock candidates have a Bayesian reading. Each is a \emph{score-driven}
