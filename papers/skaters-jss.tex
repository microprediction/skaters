...
\code{laplace} wins the per-series log-likelihood race against every baseline
decisively (85--94\% of continuous series) --- including the two exact-Gaussian
references (\pkg{SARIMAX}, statsmodels ETS) for which there is no
band-reconstruction approximation to blame --- with a single exception, a near
dead-heat with \pkg{GARCH}-$t$.

\paragraph{\pkg{GARCH}-$t$ is the tightest case.}
The \pkg{GARCH}$(1,1)$-$t$ model is purpose-built for the heavy tails and
volatility clustering of financial change-series, so it is by far the strongest
opponent on log-likelihood, and on this popularity-ranked universe (heavy in
rates, FX, and indices) the per-series race is a \emph{near dead-heat}:
\code{laplace} has the higher held-out log-likelihood on only $51\%$ of continuous
series. Its \emph{mean} continuous log-likelihood nonetheless edges ahead,
$3.10$ versus $2.92$ --- about $0.18$ nats per observation, a slightly larger gap
than the win-rate suggests because \code{laplace} avoids the worst tail
mis-specifications. That a general forecaster matches the heavy-tail specialist
series-by-series, and edges it on average, without committing to a parametric
volatility law, is the substantive point --- stated with the honest caveat that
against this one opponent it is a tie, not a rout.
