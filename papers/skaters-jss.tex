...
which \code{laplace} scores better), and additionally restrict to the $389$
...
(the unified \code{study.py sota} run).
...
\midrule
\pkg{AutoARIMA}             & \textbf{88 / 85\,\%} & 68 / 64\,\% & 2.79 \\
\pkg{AutoETS}               & \textbf{95 / 94\,\%} & 77 / 73\,\% & 2.76 \\
\pkg{SARIMAX} (statsmodels) & \textbf{89 / 86\,\%} & 64 / 59\,\% & 2.88 \\
ETS (statsmodels)           & \textbf{93 / 92\,\%} & 74 / 70\,\% & 2.87 \\
\pkg{GARCH}$(1,1)$-$t$      & 59 / 51\,\% & 40 / 33\,\% & \textbf{2.92} \\
\pkg{AutoARIMA} + conformal & \textbf{87 / 83\,\%} & 31 / 21\,\% & 2.39 \\
\pkg{AutoARIMA} + ACI       & \textbf{89 / 86\,\%} & 31 / 21\,\% & 2.52 \\
Prophet$^{\dagger}$         & \textbf{100 / 100\,\%} & 100 / 100\,\% & 1.86 \\
NF-Student$t^{\ddagger}$    & \textbf{100 / 100\,\%} & 78 / 76\,\% & 0.82 \\
\bottomrule
\end{tabular}
\caption{Per-series win-rate of \code{laplace} versus each baseline on 500
popularity-ranked daily \proglang{FRED} change-series, all and on the 389
continuous series. LL counts series where \code{laplace} has the higher held-out
log-likelihood; CRPS where it has the lower CRPS. The final column is each
baseline's mean continuous log-likelihood (\code{laplace}: 3.10). Higher is better
for LL; lower for CRPS. $^{\dagger}$Prophet refits at every step ($n=30$; a fresh
Stan fit has no online update). $^{\ddagger}$NF-Student$t$ is carried from the
prior run --- it requires \pkg{torch} and was not re-scored in this run.}
\label{tab:sota}
\end{table}
...
on CRPS --- the \pkg{ARIMA} family (\pkg{AutoARIMA} $64\%$, \pkg{SARIMAX} $59\%$),
the smoothing models (\pkg{AutoETS} $73\%$, ETS $70\%$), and Prophet --- but
\emph{loses} to \pkg{GARCH}-$t$ ($33\%$) and the conformal variants ($21\%$),
which are CRPS-optimised (or volatility-specialised) by construction. This is
...
\paragraph{Scope.}
Five hundred popularity-ranked daily change-series, one-step horizon. Multi-step
horizons and raw levels are left for later; zero-shot foundation models get their
own protocol in Section~\ref{sec:foundation}.
