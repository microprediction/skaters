"""markov: a machine-generated regime-switching signal, packaged as a
conjugation transform.

The Forecaster below was generated by a language model inside the
inspiration-simplex study (humpday repository, papers/dfo_recommender); it
is preserved verbatim as provenance. `markov_drift` wraps its one-step
conditional mean in a drift-class transform: forward subtracts a cautiously
gated fraction of the signal, `inverse_k` shifts predictives back. The gate
is a dimensionless NLMS born at zero, shrunk toward zero, capped, and asleep
on repeat-dominated (lattice) series, so composing it is close to free where
the signal carries nothing.
...
Usage (standalone)::
    ...
    f = conjugate(leaf(k=1), markov_drift(), k=1)   # a skater like any other

For the laplace-beside composition see ``benchmarks/markov_forecaster.py``
(``laplace_star_skater``) and the guide page.

Opt-in: not part of laplace's default candidate population. Benchmarked over
500 FRED daily series as an added candidate: 61% one-step log-likelihood
wins vs plain laplace, median +0.0001 nats, worst series -0.012.
"""
...
def markov_drift(eta: float = 0.02, decay: float = 0.999, cap: float = 0.5,
                 guard: float = 0.10):
    """Drift-class transform backed by the generated regime forecaster."""

    def forward(y: float, state: dict | None):
        if state is None:
            fc = Forecaster()
            fc.update(y)
            return 0.0, {"fc": fc, "theta": [0.0, 0.0], "z": None,
                         "shift": 0.0, "recent": [], "prev": y}
        state["recent"].append(1.0 if y == state["prev"] else 0.0)
        state["prev"] = y
        if len(state["recent"]) > 200:
            state["recent"].pop(0)
        asleep = (sum(state["recent"]) / max(1, len(state["recent"]))) > guard
        fc = state["fc"]
        pr = fc._predict()
        sd = math.sqrt(max(pr["var"], 1e-12))
        if state["z"] is not None:
            z = state["z"]
            err_z = (y - state["shift"]) / state["sd"] - min(cap, max(-cap,
        ...
        gap = max(-3.0, min(3.0, pr["mu"] / sd))
        z = (gap, max(-3.0, min(3.0, (fc.inflation - 1.0) * gap)))
        shift_z = min(cap, max(-cap,
                      state["theta"][0] * z[0] + state["theta"][1] * z[1]))
        shift = 0.0 if asleep else shift_z * sd
        state.update(z=z, sd=sd, shift=shift)
        fc.update(y)
        return y - shift, state

    def inverse_k(dists, state):
        sh = state.get("shift", 0.0)
        return [d.shift(sh) for d in dists]
