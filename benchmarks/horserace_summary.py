...
# Price/non-price split via the SAME a-priori asset-class classifier the study
# uses (title-based: equity/fx/commodity = price; rates/credit/other = non-price),
# so the summary's split is consistent with STUDY_EXCLUDE_PRICE.
_TITLE = ({s["id"]: s.get("title", "") for s in json.load(open(fu._UNIVERSE_JSON))}
          if os.path.exists(fu._UNIVERSE_JSON) else {})
_PRICE_CLASSES = {"equity", "fx", "commodity"}
isprice = lambda sid: fu.asset_class(_TITLE.get(sid, "")) in _PRICE_CLASSES
