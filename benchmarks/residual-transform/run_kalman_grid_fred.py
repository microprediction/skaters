"""Real-data test of laplace_scale_kalman_grid: the derived design -- a
stable, fixed grid of Kalman-filter (rho, tau^2) hypotheses for the latent
log-variance state h, combined by online likelihood weighting (a genuine
mixture, not a point estimate), scoring against the exact log(chi-sq_1)
noise law. See laplace_on_scale.py's module docstring for the derivation
and why the earlier meta-laplace designs (all point-estimate variants)
failed on real data despite passing every synthetic check.

Same local FRED sample and paired-scoring methodology as
run_scale_convolved_fred.py, so the two are directly comparable.

Usage: PYTHONPATH=src python benchmarks/residual-transform/run_kalman_grid_fred.py
"""
...
def run_series(ys):
    f = los.laplace_scale_kalman_grid()
    state = None
    lp0, lp1 = [], []
    cr0, cr1 = [], []
...
def main(dataset_name="fred", n=N_SERIES):
    ...
    print(f"[kalman-grid/{dataset_name}] {len(series)} series", flush=True)
    t0 = time.time()
    ...
    print(f"\n[kalman-grid/{dataset_name}] n={n_done} series, {time.time()-t0:.1f}s")
