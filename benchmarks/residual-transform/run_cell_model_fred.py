"""Real-data test of the homogenization cell model (benchmarks/homogenization/
cell_model.py), composed onto laplace via laplace_scale_cell_model
(cell_model_on_laplace.py). cell_model passed its synthetic gate earlier
this session (RESULTS.md, verdict "go") but real-data testing was
explicitly staged as a later phase, never done. Same paired-scoring
methodology as run_kalman_grid_fred.py / run_garch_grid_fred.py, so all
three designs are directly comparable.

Usage: PYTHONPATH=src python benchmarks/residual-transform/run_cell_model_fred.py [fred|waveform|price] [n]
"""
...
import cell_model_on_laplace as cmol       # noqa: E402
...
def run_series(ys, base_factory=None):
    f = cmol.laplace_scale_cell_model(base_factory=base_factory)
    state = None
    lp0, lp1 = [], []
    cr0, cr1 = [], []
...
def main(dataset_name="fred", n=50):
    ...
    print(f"[cell-model/{dataset_name}] {len(series)} series", flush=True)
    t0 = time.time()
    ...
    print(f"\n[cell-model/{dataset_name}] n={n_done} series, {time.time()-t0:.1f}s")
